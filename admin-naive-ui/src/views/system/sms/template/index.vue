  ...
  columns: () => [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const editBtn = () => {
          if (!hasAuth('system:sms:template:edit')) {
            return null;
          }
          return (
            <ButtonIcon
        ...
        const deleteBtn = () => {
          if (!hasAuth('system:sms:template:remove')) {
            return null;
          }
          return (
            <ButtonIcon
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard title="短信模板列表" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
      <template #header-extra>
        <TableHeaderOperation
          v-model:columns="columnChecks"
          :disabled-delete="checkedRowKeys.length === 0"
          :loading="loading"
          :show-add="hasAuth('system:sms:template:add')"
          :show-delete="hasAuth('system:sms:template:remove')"
          :show-export="false"
          @add="handleAdd"
