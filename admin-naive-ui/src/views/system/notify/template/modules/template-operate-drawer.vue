...
interface Props {
  ...
  /** the edit row data */
  rowData?: Api.System.NotifyTemplate | null;
}
...
type Model = Api.System.NotifyTemplateOperateParams;
...
type RuleKey = Extract<keyof Model, 'name' | 'code' | 'nickname' | 'content' | 'type' | 'status'>;
...
function handleParamsClose(index: number) {
  ...
  model.params = JSON.stringify(paramsArray.value);
}

function handleParamsConfirm() {
  if (paramsInputValue.value) {
    if (!paramsArray.value.includes(paramsInputValue.value)) {
      paramsArray.value.push(paramsInputValue.value);
      model.params = JSON.stringify(paramsArray.value);
    }
  }
...
function handleUpdateModelWhenEdit() {
  ...
  if (props.operateType === 'edit' && props.rowData) {
    ...
    // Parse params JSON string to array
    try {
      paramsArray.value = props.rowData.params ? JSON.parse(props.rowData.params) : [];
    } catch {
      paramsArray.value = [];
    }
  }
  ...
  <NDrawer v-model:show="visible" :title="title" display-directive="show" :width="600" class="max-w-90%">
    <NDrawerContent :title="title" :native-scrollbar="false" closable>
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
        ...
        <NFormItem label="发送人名称" path="nickname">
          <NInput v-model:value="model.nickname" placeholder="请输入发送人名称" />
        </NFormItem>
